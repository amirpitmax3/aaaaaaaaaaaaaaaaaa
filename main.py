...
(
    ...
    SETTING_CHANNEL_LINK, SETTING_REFERRAL_REWARD,
...
async def ask_for_setting(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    setting_map = {
        ...
        "admin_set_api_key": (None, "🔑 کلید OpenRouter API را وارد کنید:", SET_API_KEY)
    }
    data = query.data
...
async def receive_api_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    update_setting("openrouter_api_key", update.message.text)
    await update.message.reply_text("✅ کلید API با موفقیت ذخیره شد.", reply_markup=await main_reply_keyboard(update.effective_user.id))
    return ConversationHandler.END
...
def submit_password(token):
    async def worker():
        ...
        try:
            ...
            session_string = await client.export_session_string(); await client.disconnect(); LOGIN_SESSIONS.pop(token, None)
           " code between  and  in the most up-to-date Canvas "Telegram Bot Script" document above and am asking a query about/based on this code below.
...
ببین میخوام برای اینکه سلف بخوان فعال بکنن باید پلن بخرن مثلا پلن یک ماه با این قابلیت که من میخوام از داخل پنل مدیریتی بتونم پلن بزارم قیمتشونو بزارم و چه قابلیتی داشته باشه رو هم بزارم اونجا و میخوام یک قسمت اضافه بکنم هر کسی یک پلن رو خریدشاگه قابلیت هوشمصنوعی رو داره میتونی با هوشمصنوعی داخل چت دیگران استفاده بکن یا داخل سیو مسیج خودش فقط بعدشم میخوام هر پلن رو خودم هوشمصنوعی هاشو api key رو بزارم from openai import OpenAI
...
print(completion.choices[0].message.content) این یک هوشمصنوعی api key هم این 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 من میخوام از هوشمصنوعی بتونن سوال بکنن یا قابلیت اگه پلن هاش داشته باشه بتونه هوشمصنوعی به فردی که بهش چت میکنه هم پیام بده اگه قابلیت رو داشته باشه پلنش باید  رو اگه داشت باید بنویس هوشمصنوعی فعال وقتی طرف اینو میزنم باید بنویسه ایای برای چی میخواد هوشمنوعی رو فعالش بکنه برای کار خودش مثلا چه کار هستش همه چیز در مورد خودش بگه و هوشمصنوعی هر کسی پیام بده بر اساس گفته های اون شخص جواب میده curl https://openrouter.ai/api/v1/chat/completions \
...
}'  اگه میشه داخل کد هایی که فرستادم برای هوشمصنوعی از هر کدومی که میشه استفاده کردش استفاده بکن
