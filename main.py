...
async def self_pro_receive_contact(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with session_lock:
        # We create the client here to be used across web requests
        client = Client(name=f"login_{user_id}", api_id=API_ID, api_hash=API_HASH, in_memory=True)
        LOGIN_SESSIONS[login_token] = {
            'user_id': user_id,
            'step': 'start',
            'phone': phone_number,
            'client': client # Store client instance
        }
...
async def complete_self_pro_activation(user_id: int, session_string: str):
    # This function is now called from the web route
    loop = asyncio.get_running_loop()
    loop.create_task(application.bot.send_message(user_id, "اطلاعات شما با موفقیت تایید شد. در حال فعال‌سازی نهایی سلف‌پرو..."))
...
# --- Web Server (Flask) & HTML Templates ---
# Simple CSS for styling the login pages
WEB_CSS = """
<style>
    ...
    input { width: calc(100% - 22px); padding: 10px; margin-bottom: 20px; border: 1px solid #ddd; border-radius: 6px; }
    button { background-color: #1a73e8; color: white; padding: 12px 20px; border: none; border-radius: 6px; cursor: pointer; font-size: 16px; transition: background-color 0.3s; }
...
# HTML template for the final result page
RESULT_PAGE_HTML = f"""
<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>وضعیت ورود</title>{WEB_CSS}</head><body>
<div class="container">
...
# HTML template for the phone code entry form
LOGIN_FORM_HTML = f"""
<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>تایید ورود</title>{WEB_CSS}</head><body>
<div class="container">
    <h1>تایید ورود به تلگرام</h1>
    <p>کدی که به حساب تلگرام شما ارسال شد را وارد کنید.</p>
    <form action="/submit_code/{{ token }}" method="post">
        <input type="text" name="phone_code" placeholder="کد ورود" required>
        <button type="submit">تایید کد</button>
    </form>
</div></body></html>
"""

# HTML template for the 2FA password entry form
PASSWORD_FORM_HTML = f"""
<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>رمز عبور دو مرحله‌ای</title>{WEB_CSS}</head><body>
<div class="container">
    <h1>رمز عبور دو مرحله‌ای</h1>
    <p>لطفاً رمز عبور تایید دو مرحله‌ای حساب خود را وارد کنید.</p>
...
async def login_page(token):
    with session_lock:
        session = LOGIN_SESSIONS.get(token)
        if not session or session['step'] != 'start':
            return render_template_string(RESULT_PAGE_HTML, title="خطا", message="لینک ورود نامعتبر یا منقضی شده است.", status_class="error"), 400
    
    try:
        client = session['client']
        await client.connect()
        sent_code = await client.send_code(session['phone'])
        
        with session_lock:
            session['step'] = 'awaiting_code'
            session['phone_code_hash'] = sent_code.phone_code_hash
            
        return render_template_string(LOGIN_FORM_HTML, token=token)
    except Exception as e:
        logger.error(f"Error during login step 1 for token {token}: {e}")
        return render_template_string(RESULT_PAGE_HTML, title="خطا", message=f"خطایی رخ داد: {e}", status_class="error"), 500

@web_app.route('/submit_code/<token>', methods=['POST'])
async def submit_code(token):
    with session_lock:
        session = LOGIN_SESSIONS.get(token)
        if not session or session['step'] != 'awaiting_code':
            return render_template_string(RESULT_PAGE_HTML, title="خطا", message="جلسه نامعتبر است. لطفاً دوباره تلاش کنید.", status_class="error"), 400
    
    phone_code = request.form.get('phone_code')
    client = session['client']

    try:
        await client.sign_in(session['phone'], session['phone_code_hash'], phone_code)
        session_string = await client.export_session_string()
        
        # Run the async activation function in the bot's event loop
        asyncio.run_coroutine_threadsafe(
            complete_self_pro_activation(session['user_id'], session_string),
            application.loop
        )

        with session_lock:
            LOGIN_SESSIONS.pop(token, None) # Clean up session
        await client.disconnect()
        return render_template_string(RESULT_PAGE_HTML, title="موفقیت‌آمیز", message="ورود با موفقیت انجام شد! سلف شما در حال فعال‌سازی است. می‌توانید این صفحه را ببندید.", status_class="success")

    except SessionPasswordNeeded:
        with session_lock:
            session['step'] = 'awaiting_password'
        return render_template_string(PASSWORD_FORM_HTML, token=token)
    except (PhoneCodeInvalid, PhoneCodeExpired) as e:
        logger.warning(f"Invalid code for token {token}: {e}")
        await client.disconnect()
        return render_template_string(RESULT_PAGE_HTML, title="خطا", message="کد وارد شده اشتباه یا منقضی شده است. لطفاً دوباره تلاش کنید.", status_class="error"), 400
    except Exception as e:
        logger.error(f"Error during code submission for token {token}: {e}")
        await client.disconnect()
        return render_template_string(RESULT_PAGE_HTML, title="خطا", message=f"خطای ناشناخته: {e}", status_class="error"), 500

@web_app.route('/submit_password/<token>', methods=['POST'])
async def submit_password(token):
    with session_lock:
        session = LOGIN_SESSIONS.get(token)
        if not session or session['step'] != 'awaiting_password':
            return render_template_string(RESULT_PAGE_HTML, title="خطا", message="جلسه نامعتبر است.", status_class="error"), 400
    ...
    try:
        ...
        with session_lock:
            LOGIN_SESSIONS.pop(token, None) # Clean up
        await client.disconnect()
        return render_template_string(RESULT_PAGE_HTML, title="موفقیت‌آمیز", message="ورود با موفقیت انجام شد! سلف شما در حال فعال‌سازی است. می‌توانید این صفحه را ببندید.", status_class="success")

    except PasswordHashInvalid:
        return render_template_string(RESULT_PAGE_HTML, title="خطا", message="رمز عبور اشتباه است.", status_class="error"), 400
    except Exception as e:
        logger.error(f"Error during password submission for token {token}: {e}")
        await client.disconnect()
        return render_template_string(RESULT_PAGE_HTML, title="خطا", message=f"خطای ناشناخته: {e}", status_class="error"), 500
...
def main() -> None:
    ...
    application.loop = asyncio.get_running_loop() # Store loop for thread-safe calls
...
if __name__ == "__main__":
    if not TELEGRAM_TOKEN or "YOUR_TOKEN" in TELEGRAM_TOKEN:
        ...
        flask_thread = Thread(target=lambda: web_app.run(host='0.0.0.0', port=port, loop='asyncio'))
